...
public class ProveedorAEGui extends JPanel {
	...
	public ProveedorAEGui(Dimension d, int opcion) {

		this.setLayout(null);
		this.setLocation(0, 0);
		this.setSize((int) (d.getWidth()*0.9), (int) (d.getHeight() * 0.45));
		this.datos = Consultas.getConsultas().getProveedorCons().buscar(1, null);
	...
	public void ventana(String buscar) {
		...
		ProveedorInsertarGui prov = new ProveedorInsertarGui(ProveedorInsertarGui.getDimension(), 2);
		prov.setDato(Consultas.getConsultas().getProveedorCons().buscar(3, buscar));
		Dialogo d = new Dialogo(Marco.getGUI(), this.getSize(), prov);

	}

	public void ObtenerDatos(int codigo, String buscar) {
		this.datos = Consultas.getConsultas().getProveedorCons().buscar(codigo, buscar);
		this.modelo.setModelo(new String[] { "ruc", "nombre", "telefono", "direccion", "opcion" }, this.datos);
		this.tabla.setModelo(this.modelo, opcion, true);
		this.tabla.updateUI();
	}

	public void eliminar(String buscar) {
		Consultas.getConsultas().getProveedorCons().eliminar(buscar);
	}
	...
	public void establecerCodigo() {
		...
		if (colIndex == (this.tabla.getColumnCount() - 1) && this.opcion == 2) {
			...
			int decision = JOptionPane.showConfirmDialog(null, "¿esta seguro de eliminar el proveedor?".toUpperCase());
			if (decision == 0) {
	...
	public void atributos(int opcion) {
		...
		this.opcion = opcion;
		this.modelo = new Modelo(new String[] { "ruc", "nombre", "telefono", "direccion", "opcion" }, this.datos);
