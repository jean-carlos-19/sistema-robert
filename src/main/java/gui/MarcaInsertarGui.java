...
public class MarcaInsertarGui extends JPanel implements ActionListener {
    ...
    public MarcaInsertarGui(Dimension d, int opcion) {
    ...
    public boolean enviarDatos() {
        ...
        Marca = Procesos.getProcesos().getMarca(nombre, descripcion);

        boolean validacion = Marca.validar();
        if (validacion && opcion == 1) {
            Consultas.getConsultas().getMarcaCons().registar_actualizar("0", nombre, descripcion, 2, 1);
            ErrorClases.getErrorClases().getMarca();
            this.limpiar();
        }
        if (validacion && opcion == 2) {
            Consultas.getConsultas().getMarcaCons().registar_actualizar(this.id, nombre, descripcion, 2, 2);
            ErrorClases.getErrorClases().actualizaMarca();
            this.limpiar();
        }
        return validacion;
    }
