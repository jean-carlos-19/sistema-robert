...
public class HistorialCajaGui extends JPanel {

    private JScrollPane contenedor;
    private Tabla tabla;
    private Modelo modelo;

    public HistorialCajaGui(Dimension d) {

        this.setLayout(null);
        this.setLocation(0, 0);
        this.setSize((int) (d.getWidth() * 0.9), (int) (d.getHeight() * 0.45));

        this.modelo = new Modelo(new String[] { "n°", "mes", "ingreso", "retiro", "detalle" },
                new String[][] { { "1" } });
        this.tabla = new Tabla(modelo, new int[] { this.getSize().width / 5, this.getSize().width / 5,
                this.getSize().width / 5, this.getSize().width / 5 }, this.getSize(), new Point(0, 0), 0, false);
